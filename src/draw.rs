...
use egui::{Align, Color32, Event, Label, Layout, PointerButton, PopupAnchor, Rect, Response, RichText, Sense, Stroke, StrokeKind, Tooltip, UiKind, Vec2b};
                ...
                for (vis_col, &col) in s.vis_cols().iter().enumerate() {
                    ...
                    if resp.dragged() {
                        Tooltip::always_open(ctx.clone(), ui_layer_id, "_EGUI_DATATABLE__COLUMN_MOVE__".into(), PopupAnchor::Pointer)
                            .gap(12.0)
                            .show(|ui| ui.label(viewer.column_name(col.0)));
                    }
